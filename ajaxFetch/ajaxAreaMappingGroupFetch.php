...
JOIN company_creation c 
    ON c.company_id = agm.company_id
JOIN branch_creation b 
    ON b.branch_id = agm.branch_id
JOIN area_group_mapping_area agma 
    ON agma.group_map_id = agm.map_id
JOIN area_list_creation alc 
    ON alc.area_id = agma.area_id
WHERE alc.status = 0 ";
...
$query .= "GROUP BY agm.map_id, agm.group_name, c.company_name, b.branch_name ";
...
// First query to get filtered count
$statement = $connect->prepare($query);
$statement->execute();
$number_filter_row = $statement->rowCount();
$statement->closeCursor(); // ✅ Close cursor before next query

// Second query for actual data with pagination
